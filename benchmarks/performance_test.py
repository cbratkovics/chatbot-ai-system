...
class PerformanceTester:
    ...
    async def test_websocket_connection(self) -> float:
        ...
        try:
            ...
            async with websockets.connect(uri) as websocket:
                # Send auth
                auth_msg = json.dumps({"type": "auth_request", "data": {"token": "valid-token"}})
    ...
    async def run_concurrent_test(self, num_requests: int = 100, concurrent_users: int = 10):
        ...
        self.metrics.cache_hit_rate = self.metrics.cache_hits / max(
            self.metrics.successful_requests, 1
        )
    ...
    async def test_cache_performance(self):
        ...
        async with aiohttp.ClientSession() as session:
            for query in queries:
                ...
                try:
                    async with session.post(
                        f"{self.base_url}{self.api_prefix}/chat", json=payload
                    ) as response:
                        if response.status == 200:
                            data = await response.json()
                            if data.get("cached"):
        ...
        cache_test_metrics.cache_hit_rate = cache_test_metrics.cache_hits / max(
            cache_test_metrics.successful_requests, 1
        )
