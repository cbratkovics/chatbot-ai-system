...
class StreamingMixin:
    ...
    def _update_streaming_stats(self, chunks: int, tokens: int, duration: float):
        ...
        if duration > 0:
            ...
            self.streaming_stats["avg_tokens_per_second"] = (
                (current_avg * (total_streams - 1) + tokens_per_second) / total_streams
            )
    ...
    async def merge_streams(
        ...
        *streams: AsyncIterator[StreamChunk]
        ...
        tasks = [
            asyncio.create_task(self._consume_stream(stream, i))
        ...
        # Start producers
        producers = [
            asyncio.create_task(producer(task, i))
    ...
    async def stream_chat(
        ...
        **kwargs
        ...
        try:
            ...
            for msg in messages:
                if msg.role == "system":
                    system_message = msg.content if not system_message else f"{system_message}\n\n{msg.content}"
                else:
                    ...
                    if event.type == "content_block_delta":
                        ...
                        if hasattr(stream, 'message') and hasattr(stream.message, 'usage'):
                            usage = {
                                ...
                                "total_tokens": stream.message.usage.input_tokens + stream.message.usage.output_tokens
                            }
