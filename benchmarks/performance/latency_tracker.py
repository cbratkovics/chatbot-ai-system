...
latency_histogram = Histogram(
    "api_request_duration_seconds", "API request latency", ["endpoint", "method", "model", "status"]
)

websocket_latency = Histogram(
    "websocket_message_latency_seconds", "WebSocket message latency", ["message_type", "model"]
)

cache_latency = Histogram(
    "cache_operation_latency_seconds", "Cache operation latency", ["operation", "hit"]
)
...
model_latency = Histogram(
    "model_inference_latency_seconds", "Model inference latency", ["model", "provider"]
)
...
class LatencyTracker:
    ...
    def record_latency(
        self, metric_type: str, latency_ms: float, labels: Optional[Dict[str, str]] = None
    ):
        """
        ...
        self.time_series[metric_type].append(
            {"timestamp": datetime.now().isoformat(), "latency": latency_ms, "labels": labels or {}}
        )
        ...
        if metric_type == "api":
            ...
            cache_latency.labels(
                operation=labels.get("operation", "get"), hit=labels.get("hit", "false")
            ).observe(latency_ms / 1000)
        elif metric_type == "model":
            model_latency.labels(
                model=labels.get("model", "unknown"), provider=labels.get("provider", "unknown")
            ).observe(latency_ms / 1000)
    ...
    def get_current_metrics(self) -> Dict:
        ...
        for metric_type, window in self.windows.items():
            ...
            results["metrics"][metric_type] = {
                ...
                "max": metrics["max"],
                "percentiles": {f"p{p}": value for p, value in percentiles.items()},
    ...
    def generate_latency_graph(self, output_path: str = None) -> str:
        ...
        for idx, (metric_type, window) in enumerate(self.windows.items()):
            ...
            for p, value in percentiles.items():
                ax1.axvline(
                    value, color="red", linestyle="--", alpha=0.5, label=f"P{p}: {value:.2f}ms"
                )
            ax1.legend()
            ...
            if data["histogram"]:
                metrics["detailed"][metric_type] = {
                    ...
                    "statistics": {
                        ...
                        "stdev": statistics.stdev(data["histogram"])
                        if len(data["histogram"]) > 1
                        else 0,
                    },
                }
...
async def simulate_traffic(tracker: LatencyTracker):
    ...
    for i in range(1000):
        ...
        if random.random() > 0.6:
            ...
            tracker.record_latency(
                "model", model_latency, {"model": model, "provider": model.split("-")[0]}
            )
