...
class ReferenceUpdater:
    def __init__(self):
        self.replacements = [
            ...
            (
                r"(?<!config/docker/compose/)docker-compose\.yml",
                "config/docker/compose/docker-compose.yml",
            ),
            (
                r"(?<!config/docker/compose/)docker-compose\.test\.yml",
                "config/docker/compose/docker-compose.test.yml",
            ),
            (
                r"(?<!config/docker/compose/)docker-compose\.load\.yml",
                "config/docker/compose/docker-compose.load.yml",
            ),
            (
                r"(?<!config/docker/compose/)docker-compose\.tracing\.yml",
                "config/docker/compose/docker-compose.tracing.yml",
            ),
            # Dockerfile references
            (
                r"(?<!config/docker/dockerfiles/)Dockerfile\.multistage",
                "config/docker/dockerfiles/Dockerfile.multistage",
            ),
            # Requirements files
            (
                r"(?<!config/requirements/)requirements\.txt(?![a-z])",
