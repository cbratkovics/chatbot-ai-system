...
class DependencyHealthMonitor:
                ...
                if "upload_time_iso_8601" in file_info:
                    try:
                        dt = datetime.fromisoformat(
                            file_info["upload_time_iso_8601"].replace("Z", "+00:00")
                        )
    ...
    async def analyze_package(self, name: str, current_version: str) -> PackageHealth:
        ...
        days_behind = self.calculate_days_behind(
            current_version, latest_version, latest_release_date
        )
    ...
    async def analyze_all_dependencies(self) -> DependencyReport:
        ...
        total_score = (
            sum(
                (h.security_score * 0.4 + h.popularity_score * 0.2 + h.maintenance_score * 0.4)
                for h in package_healths
            )
            / len(package_healths)
            if package_healths
            else 0
    ...
    def _generate_recommendations(self, healths: list[PackageHealth]) -> list[str]:
        ...
        if vulnerable:
            recommendations.append(f"🔴 Fix {len(vulnerable)} security vulnerabilities immediately")

        # Check for severely outdated packages
        severely_outdated = [
            h for h in healths if h.days_behind > self.thresholds["days_outdated_critical"]
        ]
        if severely_outdated:
            recommendations.append(f"🟠 Update {len(severely_outdated)} severely outdated packages")
    ...
    def generate_markdown_report(self, report: DependencyReport) -> str:
        ...
        for health in sorted(report.package_health, key=lambda h: h.days_behind, reverse=True):
            ...
            md.append(
                f"| {health.name} | {health.current_version} | {health.latest_version} | "
    ...
    async def monitor_continuous(self, interval_hours: int = 24):
        ...
        console.print(
            f"[bold green]Starting continuous monitoring (every {interval_hours} hours)[/bold green]"
        )
