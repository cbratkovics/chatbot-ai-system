...
class MessageHandler:
    ...
    async def handle_message(
        self, websocket: WebSocket, message: Dict[str, Any], connection_id: str, **kwargs
        ...
        try:
            ...
            if not handler:
                return self._create_error_response(
                    ws_message.id, f"Unknown message type: {ws_message.type}", code=4000
                )
            ...
            return self._create_error_response(message.get("id", "unknown"), str(e), code=4001)
        except Exception as e:
            logger.error(f"Message handling error: {e}", exc_info=True)
            return self._create_error_response(
                message.get("id", "unknown"), "Internal server error", code=5000
            )
        ...
        try:
            ...
            stream_task = asyncio.create_task(
                self._stream_chat_response(
                    websocket, message.id, chat_request, connection_id, provider_factory, **kwargs
                )
            )
            ...
            return self._create_error_response(
                message.id, f"Invalid chat request: {str(e)}", code=4002
            )
        except Exception as e:
            logger.error(f"Chat handling error: {e}", exc_info=True)
            return self._create_error_response(
                message.id, "Failed to process chat request", code=5001
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if chat_request.system_prompt:
                messages.append(ChatMessage(role="system", content=chat_request.system_prompt))
            ...
            if chat_request.stream and hasattr(provider, "stream_chat"):
                ...
                async for chunk in provider.stream_chat(
                    ...
                    chunk_message = WebSocketMessage(
                        ...
                        data={
                            ...
                            if hasattr(chunk, "tokens_per_second")
                            else None,
                        },
                    )
                ...
                chunk_message = WebSocketMessage(
                    type=MessageType.STREAM,
                    id=message_id,
                    data={"chunk": full_response, "index": 0, "finished": True},
                )
            ...
            error_message = self._create_error_response(
                message_id, str(e), code=5002, details={"provider": e.provider}
            )
            ...
            error_message = self._create_error_response(
                message_id, "Stream processing failed", code=5003
            )
            await websocket.send_json(error_message.dict())
    ...
    async def _handle_ping(
        ...
        """Handle ping message."""
        return WebSocketMessage(
            type=MessageType.PONG, id=message.id, data={"timestamp": datetime.utcnow().isoformat()}
        )
    ...
    async def _handle_cancel(
        ...
        if connection_id in self.active_streams:
            ...
            return WebSocketMessage(
                type=MessageType.STATUS,
                id=message.id,
                data={"status": "cancelled", "message": "Stream cancelled successfully"},
            )
        else:
            return WebSocketMessage(
                type=MessageType.STATUS,
                id=message.id,
                data={"status": "no_stream", "message": "No active stream to cancel"},
            )

    def _create_error_response(
        self, message_id: str, error: str, code: int, details: Optional[Dict[str, Any]] = None
    ) -> WebSocketMessage:
        """Create error response message."""
        return WebSocketMessage(
            type=MessageType.ERROR,
            id=message_id,
            data={"error": error, "code": code, "details": details or {}},
        )
