    ...
    async with aiohttp.ClientSession() as session:
        ...
        for query in queries:
            ...
            async with session.post(
                ...
                json={"message": query, "session_id": session_id, "model": "gpt-4"},
            ) as resp:
                ...
                if (
                    "metadata" in result["message"]
                    and "function_call" in result["message"]["metadata"]
                ):
...
async def demo_web_search_integration():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for query in queries:
            ...
            async with session.post(
                f"{API_BASE_URL}/chat/messages", json={"message": query, "session_id": session_id}
            ) as resp:
                result = await resp.json()
...
async def demo_image_upload():
    ...
    blue_pixel_data = base64.b64decode(
