...
class Settings(BaseSettings):
    ...
    cache_compression_threshold: int = Field(1000, description="Compress responses larger than this (bytes)")
    semantic_cache_enabled: bool = Field(True, description="Enable semantic similarity caching")
    semantic_cache_threshold: float = Field(0.95, description="Similarity threshold for semantic cache")
    cache_warming_enabled: bool = Field(True, description="Enable cache warming on startup")
    cache_circuit_breaker_enabled: bool = Field(True, description="Enable circuit breaker for cache failures")
    ...
    default_anthropic_model: str = Field("claude-3-haiku-20240307", description="Default Anthropic model")
