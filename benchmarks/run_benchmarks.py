...
class BenchmarkRunner:
    ...
    def run_k6_test(self, test_name: str, config: Dict) -> Dict:
        ...
        cmd = [
            ...
            "run",
            "--out",
            "json=benchmarks/results/k6_metrics.json",
            "--summary-export",
            f"benchmarks/results/{test_name}_summary.json",
        ]
    ...
    def run_locust_test(self, config: Dict) -> Dict:
        ...
        cmd = [
            ...
            "http://localhost:8000",
            "--html",
            "benchmarks/results/locust_report.html",
            "--csv",
    ...
    def validate_performance(self, results: Dict) -> Dict:
        ...
        if "locust" in results:
            ...
            if "cache_hit_rate" in locust_metrics:
                ...
                if hit_rate >= target:
                    validation["passed"].append(
                        f"Cache hit rate: {hit_rate*100:.1f}% >= {target*100}%"
                    )
                else:
                    validation["failed"].append(
                        f"Cache hit rate: {hit_rate*100:.1f}% < {target*100}%"
    ...
    async def run(self) -> Dict:
        ...
        if self.config["k6"]["websocket_test"]["enabled"]:
            self.results["k6_websocket"] = self.run_k6_test(
                "k6_websocket", self.config["k6"]["websocket_test"]
            )
...
async def main():
    ...
    parser.add_argument(
        "--compare-baseline", action="store_true", help="Compare with baseline results"
    )
    parser.add_argument(
