...
def test(verbose, coverage, unit, integration, e2e):
    ...
    try:
        ...
        click.echo(
            click.style("pytest not found. Install with: pip install pytest pytest-cov", fg="red"),
            err=True,
        )
        sys.exit(1)
...
def runserver(host, port, reload, workers):
    ...
    try:
        ...
        click.echo(
            click.style("uvicorn not found. Install with: pip install uvicorn", fg="red"), err=True
        )
        sys.exit(1)
...
"""
    ...
    try:
        ...
        code.interact(
            local=namespace,
            banner="AI Chatbot System Shell (install ipython for better experience)",
        )
...
def lint(check):
    ...
    tools = [
        (
            "black",
            ["black", "--check" if check else None, "api/", "tests/", "benchmarks/"],
            "Code formatting",
        ),
        (
            "isort",
            ["isort", "--check-only" if check else None, "api/", "tests/", "benchmarks/"],
            "Import sorting",
        ),
