...
class ClaimValidator:
    ...
    def validate_monitoring_setup(self) -> tuple[bool, str]:
        ...
        if existing == len(monitoring_files):
            ...
            return (
                False,
                f"Partial monitoring setup ({existing}/{len(monitoring_files)} components)",
            )
        else:
    ...
    def get_recommendations(self) -> list[str]:
        ...
        if "P95 Latency <200ms" in self.failed:
            recommendations.append(
                "Optimize API response times - consider caching, query optimization, or scaling"
            )

        if "30% Cost Reduction" in self.failed:
            recommendations.append(
                "Improve cache hit rate - analyze common queries and optimize cache key generation"
            )

        if "100+ Concurrent Users" in self.failed:
            recommendations.append(
                "Scale WebSocket infrastructure - increase connection pool size and optimize message handling"
            )
        ...
        if "Documentation" in self.failed:
            recommendations.append(
                "Complete documentation - ensure all architecture and API docs are up to date"
            )
...
def main():
    ...
    if "--run-benchmarks" in sys.argv:
        ...
        result = subprocess.run(
            ["python", "scripts/utils/manage.py", "benchmark"], capture_output=True, text=True
        )
