...
class DependencyVisualizer:
    ...
    def analyze_graph_metrics(self, G: nx.DiGraph) -> dict:
        ...
        metrics["most_depended_on"] = sorted(in_degree.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        metrics["most_dependencies"] = sorted(out_degree.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
